  ...
  def train(self, trainData, maxEpochs, learnRate, L1=False, lamda=0.0):
    ...
    while epoch < maxEpochs:
      ...
      for ii in range(numTrainItems):
        ...
        for k in range(self.no):
          obGrads[k] = oSignals[k] * 1.0  # no regularization for biases
          ...
          for j in range(self.nh):
            ...
            if L1 == True:
              if self.ihWeights[i,j] > 0.0:
                ihGrads[i,j] += lamda
              elif self.ihWeights[i,j] < 0.0:
                ihGrads[i,j] -= lamda

        # 6. compute hidden node bias gradients using hidden signals
        for j in range(self.nh):
          hbGrads[j] = hSignals[j] * 1.0  # no regularization
...
def main():
  print("\nBegin NN L1 regularization demo \n")
  
  print("Generating dummy training and test data \n")
  genNN = NeuralNetwork(10, 15, 4, 0)  # a 10-15-4 NN with seed = 0 (init wts, train order; not used
  ...
  genNN.setWeights(genWeights)  # wts for the random dummy data
  
  dummyTrainData = makeData(genNN, numRows=200, inputsSeed=16)  # 200 rows train data
  dummyTestData = makeData(genNN, 40, 18)  # 40 rows of test data, different input values
  ...
  print("\nStarting training without L1 regularization")
  ...
  print("\nAccuracy on train data no L1 regularization = %0.4f " % accTrain)
  print("Accuracy on test data no L1 regularization  = %0.4f " % accTest)
  
  L1_rate = 0.001
  nn = NeuralNetwork(numInput, numHidden, numOutput, seed=0)  # reset the NN
  print("\nStarting training with L1 regularization ( rate = %0.3f ) " % L1_rate)
  nn.train(dummyTrainData, maxEpochs, learnRate, L1=True, lamda=L1_rate)  # with regularization
  print("Training complete")
  
  accTrain = nn.accuracy(dummyTrainData)
  accTest = nn.accuracy(dummyTestData)
  
  print("\nAccuracy on train data with L1 regularization = %0.4f " % accTrain)
  print("Accuracy on test data with L1 regularization = %0.4f " % accTest)
